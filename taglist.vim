...
function! s:Tlist_Parse_Tagline(tag_line)
    ...
    let has_static = s:Tlist_Extract_Static(a:tag_line)
    if has_static
    	let ttxt = ' [s]'. ttxt
    else
    	let ttxt = '    '. ttxt
...
function! s:Tlist_Process_File(filename, ftype)
    ...
    let ctags_args = ' -f - --format=2 --excmd=pattern --fields=nksT '
...
function! s:Tlist_Extract_Static(tag_line)
    " check the tag is static
    " we look at the initial token at the prototype field

    let start = strridx(a:tag_line, 'static:')
    if start == -1
    	return 0
    endif

    let str = strpart(a:tag_line, start + 7, 1)
    if str =="1"
    	return 1
    else
    	return 0
